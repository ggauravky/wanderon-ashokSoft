...
import { getDestinationWeather } from '../services/travelKnowledgeService.js';

const WeatherBadge = ({ weather: initialWeather, location = '', size = 'sm', showCondition = true, className = '' }) => {
  const [currentWeather, setCurrentWeather] = React.useState(initialWeather);
  ...
  React.useEffect(() => {
    if (!location) return;
    const handleUpdate = () => {
      const updated = getDestinationWeather(location);
      if (updated) setCurrentWeather(updated);
    };
