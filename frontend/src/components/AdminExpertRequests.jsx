          ...
          >
            ...
            >
              ...
              <div className="p-6 space-y-6 overflow-y-auto flex-1">
                ...
                {/* AI PLANNER EXPEDITION BLUEPRINT & PREFERENCES */}
                {(selectedLead.source === 'ai_planner_booking' || selectedLead.origin || selectedLead.stayPreference || selectedLead.itinerarySnapshot) && (
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-black uppercase tracking-wider text-emerald-800 flex items-center gap-1.5">
                        <Sparkles size={12} className="text-emerald-600" />
                        AI Planner Route & Stay Blueprint
                      </span>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {selectedLead.origin && (
                          <div className="bg-white p-2.5 rounded-xl border border-emerald-100">
                            <span className="text-[10px] text-slate-400 font-bold block">Origin City</span>
                            <span className="font-extrabold text-slate-900">{selectedLead.origin}</span>
                          </div>
                        )}
                        {selectedLead.duration && (
                          <div className="bg-white p-2.5 rounded-xl border border-emerald-100">
                            <span className="text-[10px] text-slate-400 font-bold block">Duration</span>
                            <span className="font-extrabold text-slate-900">{selectedLead.duration}</span>
                          </div>
                        )}
                        {selectedLead.stayPreference && (
                          <div className="bg-white p-2.5 rounded-xl border border-emerald-100">
                            <span className="text-[10px] text-slate-400 font-bold block">Stay Vibe</span>
                            <span className="font-extrabold text-slate-900">{selectedLead.stayPreference}</span>
                          </div>
                        )}
                        {selectedLead.roomStyle && (
                          <div className="bg-white p-2.5 rounded-xl border border-emerald-100">
                            <span className="text-[10px] text-slate-400 font-bold block">Room Type</span>
                            <span className="font-extrabold text-slate-900">{selectedLead.roomStyle}</span>
                          </div>
                        )}
                        {selectedLead.dietaryPreference && (
                          <div className="bg-white p-2.5 rounded-xl border border-emerald-100">
                            <span className="text-[10px] text-slate-400 font-bold block">Dining</span>
                            <span className="font-extrabold text-slate-900">{selectedLead.dietaryPreference}</span>
                          </div>
                        )}
                        {selectedLead.transitPreference && (
                          <div className="bg-white p-2.5 rounded-xl border border-emerald-100">
                            <span className="text-[10px] text-slate-400 font-bold block">Transit Mode</span>
                            <span className="font-extrabold text-slate-900">{selectedLead.transitPreference}</span>
                          </div>
                        )}
                        {selectedLead.pacePreference && (
                          <div className="bg-white p-2.5 rounded-xl border border-emerald-100">
                            <span className="text-[10px] text-slate-400 font-bold block">Pace & Rhythm</span>
                            <span className="font-extrabold text-slate-900">{selectedLead.pacePreference}</span>
                          </div>
                        )}
                        {selectedLead.budgetTier && (
                          <div className="bg-white p-2.5 rounded-xl border border-emerald-100">
                            <span className="text-[10px] text-slate-400 font-bold block">Budget Tier</span>
                            <span className="font-extrabold text-emerald-800">{selectedLead.budgetTier}</span>
                          </div>
                        )}
                      </div>

                      {/* Itinerary Stops Snapshot if available */}
                      {selectedLead.itinerarySnapshot?.stops && Array.isArray(selectedLead.itinerarySnapshot.stops) && (
                        <div className="pt-2 border-t border-emerald-200/60 space-y-1.5">
                          <span className="text-[10px] font-black uppercase text-slate-500 block">
                            Planned Day Stops:
                          </span>
                          <div className="space-y-1 max-h-36 overflow-y-auto pr-1">
                            {selectedLead.itinerarySnapshot.stops.map((st, i) => (
                              <div key={i} className="bg-white p-2 rounded-lg border border-emerald-100 text-[11px] flex items-center justify-between">
                                <span className="font-bold text-slate-800">Day {st.day || i + 1}: {st.name || st.stage}</span>
                                {st.distance && <span className="text-[10px] text-slate-400 font-mono">{st.distance}</span>}
                              </div>
                            ))}
                          </div>
                        </div>
