...
export default function TravelDocumentCard({ document: item }) {
  ...
  useEffect(() => {
    let cancelled = false;
    if (isImage) { setPreview(item.secureUrl); setStatus('ready'); }
    else if (isPdf) {
      import('../utils/pdfDocumentPreview.js').then(({ renderPdfFirstPage }) => renderPdfFirstPage(item.secureUrl))
        .then((url) => { if (!cancelled) { setPreview(url); setStatus('ready'); } })
  ...
  return <article className="pdf-document-card" data-pdf-document={status}>
    ...
    <div className="pdf-document-preview">
      {preview && isPdf ? <img src={preview} alt={`Preview of ${item.title}`} data-pdf-image="ready" onError={() => { setPreview(''); setStatus('failed'); }} />
    ...
    <div className="pdf-document-link-row"><span>{item.mimeType || 'Document'}</span><a href={item.secureUrl} target="_blank" rel="noopener noreferrer">View original document</a>{qr && <img src={qr} alt="QR code for original document" className="pdf-document-qr" />}</div>
  </article>;
}
