...
export const TransportCard = ({ item, showMedia = true, mediaLimit = 1 }) => {
  ...
  return <article className="pdf-transport-card" data-pdf-flow-item="transport"><div className="pdf-transport-heading"><Icon size={19} aria-hidden="true" /><div><div className="pdf-card-kicker">{item.mode || 'Transport'}</div><h3>{item.title}</h3></div></div>{showMedia && media.length > 0 && <><div className={`pdf-transport-media ${media.length > 1 ? 'has-secondary' : ''}`}>{media.map((entry) => <PdfImage key={entry.url} src={entry.url} alt={entry.altText || item.title} className="pdf-transport-image" fallbackLabel="Vehicle image unavailable" />)}</div>{hero.caption && <p className="pdf-image-caption">{hero.caption}</p>}</>}<p className="pdf-route">{[item.pickup, item.drop].filter(Boolean).join(' → ') || 'Route to be confirmed'}</p><div className="pdf-card-meta">{[item.vehicle, item.reference, item.cabinClass, item.seatDetails, item.baggage, item.schedule.departureDate ? formatQuotationV2Date(item.schedule.departureDate) : ''].filter(Boolean).map((value) => <span key={value}>{value}</span>)}</div>{item.notes && <p className="pdf-copy">{item.notes}</p>}{item.documents.length > 0 && <p className="pdf-document-available">{item.documents.length} ticket{item.documents.length === 1 ? '' : 's'} / voucher{item.documents.length === 1 ? '' : 's'} in Travel Documents</p>}</article>;
};
