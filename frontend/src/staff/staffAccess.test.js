...
test('administrators see every department workspace', () => {
  const expected = ['Overview', 'Admin Overview', 'Trips', 'Existing Admin Tools', 'Sales Overview', 'Expert Requests', 'Quotations', 'Bookings', 'Management Workspace'];
...
test('all department links stay inside the canonical staff application', () => {
  ...
  assert.deepEqual(workspacePaths, ['/staff/admin', '/staff/admin/trips', '/staff/admin/legacy', '/staff/sales', '/staff/sales/expert-requests', '/staff/sales/quotations', '/staff/sales/bookings', '/staff/management']);
});
