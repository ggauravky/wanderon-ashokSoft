...
import { getAiSuggestionSourceLabel, hasMeaningfulSuggestion } from './aiAssistUi.js';
...
export default function AiSuggestionDialog({ proposal, onApply, onClose }) {
  ...
  if (!proposal || !hasMeaningfulSuggestion(proposal.suggestion)) return null;
  const multiple = proposal.field === 'policies.all';
  const listSuggestion = Array.isArray(proposal.suggestion);
  const selectedRequired = multiple || listSuggestion;
  const sourceLabel = getAiSuggestionSourceLabel(proposal);
  return <div className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-950/60 p-4" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section role="dialog" aria-modal="true" aria-label="Review writing suggestion" className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-lg bg-white p-5 shadow-2xl">
      <div className="flex items-center justify-between gap-3"><div className="flex flex-wrap items-center gap-2"><h2 className="text-base font-semibold text-slate-950">Review {proposal.label || 'suggestion'}</h2><span className="rounded-full bg-slate-100 px-2 py-1 text-xs font-semibold text-slate-600">{sourceLabel}</span></div><button ref={closeRef} type="button" aria-label="Close suggestion" onClick={onClose}><X size={18} /></button></div>
