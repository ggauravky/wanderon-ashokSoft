...
const QuickLink = ({ to, icon: Icon, title, description }) => to ? <Link to={to} className="group rounded-xl border border-slate-200 bg-white p-4 shadow-sm transition hover:border-slate-300 hover:shadow"><Icon size={18} className="text-emerald-700"/><div className="mt-4 flex items-center justify-between"><h3 className="text-sm font-semibold text-slate-950">{title}</h3><ArrowRight size={15} className="text-slate-400 transition group-hover:translate-x-0.5"/></div><p className="mt-1 text-xs leading-5 text-slate-500">{description}</p></Link> : <div className="rounded-xl border border-slate-200 bg-slate-50 p-4"><Icon size={18} className="text-slate-500"/><h3 className="mt-4 text-sm font-semibold text-slate-800">{title}</h3><p className="mt-1 text-xs leading-5 text-slate-500">{description}</p></div>;
...
export default function MarketingOverview() {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      const data = await getMarketingDashboardApi();
      setDashboard(data.dashboard);
    } catch (err) {
      setError(getApiErrorMessage(err, 'Unable to load the Marketing overview.'));
    } finally {
      setLoading(false);
    }
  }, []);
