...
const TeamAnalyticsWorkspace = () => {
  ...
  useEffect(() => {
    ...
    getTeamAnalyticsMembersApi(category).then((response) => { if (active) setMembers(response.members || []); }).catch((loadError) => { if (active) setMemberError(loadError.message || 'Unable to load team members.'); }).finally(() => { if (active) setLoadingMembers(false); });
    return () => { active = false; };
  }, [category]);
  ...
  return <div className="space-y-6">
    ...
    {memberError && <section className="rounded-xl border border-rose-200 bg-rose-50 px-5 py-4 text-sm text-rose-800"><strong>Unable to load team members.</strong> {memberError}</section>}
