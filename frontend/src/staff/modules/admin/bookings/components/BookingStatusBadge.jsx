...
const BookingStatusBadge = ({ status }) => {
  ...
  return <span className={`inline-flex rounded-full border px-2 py-0.5 text-[11px] font-semibold ${STYLES[normalized] || 'border-slate-200 bg-slate-50 text-slate-600'}`}>{normalized ? normalized.replaceAll('_', ' ') : 'Unknown'}</span>;
};
