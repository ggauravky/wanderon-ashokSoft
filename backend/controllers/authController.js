...
const invalidCredentials = (res) => res.status(401).json({ message: 'Invalid email or password.' });
...
export const addBooking = async (req, res) => {
  try {
    ...
    const newBooking = { id: `WL-${Math.floor(100000 + Math.random() * 900000)}`, bookingDate: new Date().toISOString().split('T')[0], status: 'Confirmed', ...req.body };
    if (!Array.isArray(user.bookedTrips)) user.bookedTrips = [];
    user.bookedTrips.unshift(newBooking);
    await user.save();
    return res.status(201).json(newBooking);
  } catch (error) {
    console.error('Profile booking update failed:', error.message);
    return res.status(500).json({ message: 'Unable to save booking.' });
  }
};
...
export const cancelUserBooking = async (req, res) => {
  try {
    ...
    user.bookedTrips = (user.bookedTrips || []).map((booking) => booking.id === bookingId ? { ...booking, status: 'Cancelled' } : booking);
    await user.save();
    return res.json({ message: 'Booking cancelled successfully', bookingId });
  } catch (error) {
    console.error('Profile booking cancellation failed:', error.message);
