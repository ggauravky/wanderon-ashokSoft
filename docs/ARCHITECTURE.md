...
- **Operations Phase 4 closure and finance layer**: `OperationalCost` is the INR authority for actual journey expenses and becomes financially immutable after finalization. `OperationalSettlement` separately records external Vendor payments against finalized costs, so payment records never double-count expense. `OperationalFeedback` stores one optional factual record per Operational Trip and Booking, while `OperationalTripClosure` stores centrally derived readiness, aggregate final snapshots, reasoned close/reopen history, and the closed-trip write boundary. Reports use explicit travel, incurred, paid, and closed date bases and label booked revenue minus finalized operational cost as gross operational contribution—not accounting net profit.
...
- **Operations security boundary**: Operations APIs and internal documents are limited to `operations`, `admin`, and `super_admin`, with trip reopening reserved for Admin and Super Admin. Source handoff DTOs remove supplier costs, margins, tokens, and other commercial/security internals. Communication and Incident notes are never exposed through public routes. Phase 4 is an internal factual Operations ledger only: it does not execute bank payouts, automate customer surveys, alter Creator finance, or modify customer Razorpay flows.
